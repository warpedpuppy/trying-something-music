...
import { SLOT_PX, CURSOR_FRAC, resumedStartTime } from "../lib/playAlongTiming";
import {
  loadPlayAlongConfig,
  type PlayAlongConfig,
} from "../lib/playAlongConfig";
import { getSession, updatePlayAlongBest } from "../lib/localDb";
import { SLOT_COUNT, EXTRA_SLOTS } from "./play-along/constants";
import { NotationBlock } from "./play-along/NotationBlock";
import {
  GameOverScreen,
  type GameOverScreenProps,
} from "./play-along/GameOverScreen";
...
export function PlayAlong() {
  usePageTitle(
    "Play Along",
    "Sheet music scrolls past at a steady tempo — tap along and feel the rhythm. Free, no account needed.",
  );
  ...
  const [reviewMeasure, setReviewMeasure] = useState<GeneratedMeasure | null>(
    null,
  );
  ...
  function closeReviewModal() {
    ...
    if (
      ...
      (phaseRef.current === "playing" || phaseRef.current === "static")
    ) {
      const beats = reelRef.current[0]?.timeSigTop ?? 4;
      window.setTimeout(() => {
        tickEngine.startMetronome(bpmRef.current, undefined, undefined, beats);
  ...
  useEffect(() => {
    const handleVisibility = () => {
      if (document.hidden) tickEngine.cancelAll();
    };
  ...
  if (phase === "welcome") {
    return (
      <WelcomeScreen
        onStart={(bpm, m) => startStatic(bpm, m)}
        cfg={cfg}
        onCfgChange={setCfg}
        initialMode={mode}
      />
  ...
  return (
    <div className="pa-playing">
      ...
      >
        <div
          ...
          style={{
            ...
            transform:
              phase === "static" ? `translateX(${staticTX}px)` : undefined,
          }}
        >
      ...
      <div className="pa-controls-row">
        ...
        >
          <span className="pa-tap-btn-label">
            {phase === "static" ? "START" : "TAP"}
          </span>
      ...
      <div className="pa-secondary-controls">
        ...
        <button
          type="button"
          className="pa-stop-btn-pill"
          onClick={stopToWelcome}
        >
          ■ Stop
        </button>
      </div>
