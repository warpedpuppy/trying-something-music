...
export function WelcomeScreen({
  ...
  const speedLabel =
    localCfg.startBpm < 66
      ? "Slow"
      : localCfg.startBpm < 108
        ? "Moderate"
        : localCfg.startBpm < 168
          ? "Fast"
          : "Very fast";
  ...
  return (
    <div className="pa-stage pa-welcome">
      ...
      <ul className="pa-welcome-bullets">
        ...
        <li>
          Hits turn green, misses turn orange —{" "}
          {localCfg.consecutiveMissesReset} consecutive misses ends the game
        </li>
        <li>The orange ▼ marks each downbeat and pulses to keep your place</li>
        <li>
          Every {localCfg.bpmIncreaseAfterMeasures} perfect measures, tempo
          rises by {localCfg.bpmIncreaseAmount} BPM, up to {localCfg.bpmCap}
        </li>
        <li>
          <strong>
            Tap any measure while playing to pause and hear it played correctly
          </strong>
        </li>
      </ul>
      ...
      <div className="pa-mode-section">
        ...
        <div className="pa-mode-picker">
          {(["easy", "intermediate", "advanced"] as DifficultyMode[]).map(
            (m) => (
              <button
                key={m}
                type="button"
      ...
      <p className="pa-tempo-row">
        Starting tempo:{" "}
        <span className="pa-tempo-inline-bpm">{localCfg.startBpm}</span> BPM
        <span className="pa-tempo-inline-label">
          {" "}
          · <em>{speedLabel}</em>
        </span>
      </p>
      ...
      <button
        type="button"
        className="btn-primary pa-cta"
        onClick={() => onStart(localCfg.startBpm, selectedMode)}
      >
        Let's go
      </button>
      ...
      >
        <span className={`pa-settings-caret${settingsOpen ? " open" : ""}`}>
          ›
        </span>{" "}
        Settings
      </button>
          ...
          <section className="pa-settings-section">
            ...
            <label className="pa-settings-row">
              ...
              <input
                ...
                onChange={(e) =>
                  update({ bpmIncreaseAfterMeasures: Number(e.target.value) })
                }
              />
            </label>
            <label className="pa-settings-row">
              ...
              <input
                ...
                onChange={(e) =>
                  update({ bpmIncreaseAmount: Number(e.target.value) })
                }
              />
            </label>
          </section>
          ...
          <section className="pa-settings-section">
            ...
            <label className="pa-settings-row">
              ...
              <input
                ...
                onChange={(e) =>
                  update({ consecutiveMissesReset: Number(e.target.value) })
                }
              />
            </label>
          </section>
