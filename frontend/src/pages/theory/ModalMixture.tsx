...
const MM_MODES: QuizMode<MMQuestion>[] = [
  { id: 'roman-to-chord', label: 'Roman → Chord', pool: KEYS.map(() => mmPick()), hint: 'Borrowed from parallel minor · ♭VII = most common in rock · iv = most emotive' },
...
function Quiz() {
  return (
    <TheoryQuiz<MMQuestion>
      modes={MM_MODES}
      pickQuestion={(_pool, excludeKey) => mmPick(excludeKey as KeyName | undefined)}
      getExcludeKey={(q) => q.key}
      pickChoices={(q, _pool, modeId) => mmChoices(modeId as MMMode, q)}
      getAnswer={(q, modeId) => modeId === 'roman-to-chord' ? q.chord.symbol : q.chord.roman}
      renderQuestion={(q, modeId, selected, answer) => (
        <div className={`theory-q-card${selected !== null ? (selected === answer ? ' theory-q-correct' : ' theory-q-wrong') : ''}`}>
          {modeId === 'roman-to-chord' ? (
            <>
              <div className="theory-q-main" style={{ fontFamily: 'Georgia, serif' }}>{q.chord.roman}</div>
              <div className="theory-q-sub">What is this borrowed chord in <strong>{q.key} major</strong>?</div>
            </>
          ) : (
            <>
              <div className="theory-q-main" style={{ fontFamily: 'Georgia, serif' }}>{q.chord.symbol}</div>
              <div className="theory-q-sub">What Roman numeral is this borrowed chord in <strong>{q.key} major</strong>?</div>
            </>
          )}
        </div>
      )}
